...
situacion_final<-function(x) {
  media=mean(x)
  if(media >= 4) { # cual es el promedio que aprueba?
    print("Aprobado")
  } else {
    print("Reprobado")
  }
...
while (speed >= 30) {  # mientras la velocidad sea mayor a 30
  print(paste("Tu velocidad es",speed))
  if (speed > 48) { # la velocdad es mayor a 48
  print(paste("Reduce la velocidad de inmediato!"))
  speed <- speed - 11 # restemos 11 a la velocidad
  } else {
  print(paste("Baja la velocidad!"))  
  speed <- speed - 6 # restemos 6 a velocidad
  }
}
```
...
url <- "https://github.com/edojimenez/r_datascience/blob/main/titanic.xls?raw=true"
destfile <-"/Users/eduardojimenez/titanic.xls"
download.file(url, destfile)

titanic <- read_excel(destfile)
```
...
ggplot(titanic, aes(x=Age)) + geom_histogram()  + theme_bw()
```
...
## Faceting
Si notan bien en el gráfico anterior usamos facet_grid, Faceting divide el gráfico en subplots, basado en el valor de una o más variables discretas.
